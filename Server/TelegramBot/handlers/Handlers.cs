    ...
    public class Handlers
    {
        ...
        public static async Task OnStateChanged(long chat_id, MyDbContext db, Message message, TelegramBotValuesModel cache)
        {
            ...
            switch (cache.State)
            {
                case PlannerStates.NONE:
                    {
                        await Bot_.SendTextMessageAsync(chat_id, choose_lang, replyMarkup: ReplyKeyboards.SendLanguages());
                ...
                case PlannerStates.MAIN_MENU:
                    {
                        ...
                        await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["NONE"], replyMarkup: ReplyKeyboards.MainMenu(cache.Lang));
                        break;
                    }
                case PlannerStates.FAVORITES:
                    {
                        ...
                        {
                            await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["EMPTY_LIST"]);
                            break;
                        }
                ...
                case PlannerStates.SELECT_FAVORITES:
                    {
                        ...
                        if (staffInfo is not null)
                        {
                            ...
                            ReplyKeyboardMarkup back = new(new[] { new KeyboardButton(Program.langs[cache.Lang]["back"]) }) { ResizeKeyboard = true };
                            await CalendarKeyboards.SendCalendar(Bot_, chat_id, back, cache.Lang);
                        }
                        break;
                    }
                ...
                case PlannerStates.SETTINGS:
                    {
                        await Bot_.SendTextMessageAsync(chat_id, msg, replyMarkup: ReplyKeyboards.Settings(cache.Lang));
                        break;
                    }
                ...
                case PlannerStates.CHANGE_NAME:
                    {
                        ...
                        cache.State = PlannerStates.SETTINGS;
                        await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["CONFIRMED"] + msg);
                        await Bot_.SendTextMessageAsync(chat_id, msg, replyMarkup: ReplyKeyboards.Settings(cache.Lang));
                        break;
                    }
                case PlannerStates.CHANGE_PHONE:
                    {
                        if (Utils.Utils.CheckPhone(msg))
                        {
                            ...
                            await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["CONFIRMED"]);
                            cache.State = PlannerStates.SETTINGS;
                            await Bot_.SendTextMessageAsync(chat_id, msg, replyMarkup: ReplyKeyboards.Settings(cache.Lang));
                        }
                        else
                        {
                            ...
                            await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["ON_CHANGE_PHONE"], parseMode: ParseMode.Html, replyMarkup: back);
                            return;
                        }     
                        break;
                    }
                ...
                case PlannerStates.PHONE:
                    {
                        ...
                        {
                            await ReplyKeyboards.RequestContactAsync(Bot_, chat_id, cache.Lang);
                            return;
                        }
                        cache.Phone = phoneNumber;
                        cache.State = PlannerStates.USERNAME;
                        ReplyKeyboardMarkup back = new(new[] { new KeyboardButton(Program.langs[cache.Lang]["back"]) }) { ResizeKeyboard = true };
                        await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["SEND_FULL_NAME"], replyMarkup: back);
                        return;
                    }
                case PlannerStates.USERNAME:
                    {
                        cache.UserName = msg;
                        await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["BID"]);
                        await DbManipulations.FinishProcessAsync(chat_id, cache, db);
                        await DbManipulations.RegistrateUserPlanner(chat_id, cache, db);
                        await Utils.Utils.OnFinish(cache, Bot_, db);
                        if (!await DbManipulations.CheckFavorites(db, cache.Staff, chat_id))
                        {
                            cache.State = PlannerStates.ADD_FAVORITES;
                            await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["ADD_FAVORITES"], replyMarkup: ReplyKeyboards.SendConfirmKeyboards(cache.Lang));
                            break;
                        }
                ...
                case PlannerStates.ADD_FAVORITES: 
                    {
                        ...
                        if(msg == Program.langs[cache.Lang]["YES"])
                        {
                            ...
                            await Bot_.SendTextMessageAsync(chat_id, 
        ...
        public static async Task OnCommands(TelegramBotValuesModel cache, string msg, long chat_id, MyDbContext db)
        {
                    ...
                    case PlannerStates.PHONE:
                        {
                            ...
                            await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["CUZY_TIME"], replyMarkup: await TimeKeyboards.SendTimeKeyboards(db, cache));
                            return;
                        }
                    case PlannerStates.USERNAME:
                        {
            ...
            if (Commands.change_name.Contains(msg))
            {
                ...
                {
                    await Bot_.SendTextMessageAsync(chat_id, Program.langs[cache.Lang]["SHOULD_REGISTR"]);
                    cache.State = PlannerStates.MAIN_MENU;
                    return;
                }
                ReplyKeyboardMarkup back = new(new[] { new KeyboardButton(Program.langs.Get(cache.Lang, "back")) }) { ResizeKeyboard = true };
