...
namespace waPlanner.TelegramBot.handlers.users
{
    public class OnUsersStateChanged
    {
        public static async Task OnStateChange(long chat_id, MyDbContext db, ITelegramBotClient Bot_, Message message,
            ReplyKeyboardMarkup back, TelegramBotValuesModel cache)
        {
            string message_for_user = "";
            string msg = message.Text;

            (List<IdValue> list, bool Is) category = (null, false);
            int category_id = 0;
            ...
            switch (cache.State)
            {
                case PlannerStates.NONE:
                    {
                        var services = await DbManipulations.GetRootList(db);
                        var servicesButtons = ReplyKeyboards.GetMenu(services);
                ...
                case PlannerStates.CATEGORY:
                    {
                        category_id = await DbManipulations.GetCategoryIdByName(db, msg);
                        message_for_user = "Выберите категорию";
                        cache.CategoryId = category_id;
                        category = await DbManipulations.GetChildCategory(db, category_id, cache);
                        break;
                    }
                default:
                    break;
            }
            var buttons = ReplyKeyboards.GetMenu(category);
            buttons.Add(new List<KeyboardButton> { new KeyboardButton("⬅️Назад") });
            ...
            //        case PlannerStates.NONE:
            //            {
            //                category = await DbManipulations.GetRootList(db);
            //                cache.State = PlannerStates.CATEGORY;
            //                break;
            //            }
            //        case PlannerStates.CATEGORY:
            //            {
            //                cache.CategoryId = await DbManipulations.GetCategoryIdByName(db, msg);
            //                category = await DbManipulations.GetChildCategory(db, cache.CategoryId, cache);
            //                break;
            //            }
            //        default:
            //            break;
            ...
            //public static async Task OnMenuAction(long chat_id, int menu, MyDbContext db, TelegramBotValuesModel cache, string msg)
            //{
            //    (List<IdValue> list, bool Is) category = (null, false);
            ...
            //        case PlannerStates.CATEGORY:
            //            {
            //                cache.CategoryId = await DbManipulations.GetCategoryIdByName(db, msg, menu);
            //                break;
            //            }
            //        default:
            //            break;
